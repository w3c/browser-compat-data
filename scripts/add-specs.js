...
const scrapeMDNSpecTable = (html, feature, mdnURL, deprecated, nonstandard) => {
  let specURLs = [];
  ...
  Array.from(items).forEach(item => {
    ...
    if (link) {
      ...
      if (!specURLs.includes(specURL)) {
        const dupe = specURLs.find(element =>
          element.endsWith(`${parsedURL.hash}`),
        );
        if (dupe) {
          if (
            dupe.includes('/CSS2/') ||
            dupe.includes('pointerevents1') ||
            dupe.includes('/TR/IndexedDB/') ||
            dupe.includes('/TR/user-timing/') ||
            dupe.includes('/TR/performance-timeline/') ||
            dupe.includes('/TR/hr-time-1/') ||
            dupe.includes('/TR/SVG11/') ||
            dupe.includes('/TR/touch-events/') ||
            dupe.includes('/css-shapes/') ||
            dupe.includes('css-sizing-3') ||
            dupe.includes('css-images-3') ||
            dupe.includes('css-values-3') ||
            dupe.includes('css-writing-modes-3') ||
            dupe.includes('/TR/CSP2/') ||
            dupe.includes('/webappsec-csp/2/')
          ) {
            if (
              !specURL.includes('/CSS2/') &&
              !specURL.includes('/TR/CSP2/') &&
              !specURL.includes('/webappsec-csp/2/')
            ) {
              specURLs.splice(specURLs.indexOf(dupe), 1);
              specURLs.push(specURL);
            }
          } else if (
            dupe.includes('the-cssmediarule-interface') ||
            dupe.includes('extensions-to-cssrule-interface') ||
            dupe.includes('extensions-to-the-document-interface') ||
            dupe.includes('extensions-to-the-element-interface') ||
            dupe.includes('extensions-to-the-mouseevent-interface') ||
            dupe.includes('extensions-to-the-range-interface') ||
            dupe.includes('extensions-performance-interface') ||
            dupe.includes('keyboard-interface')
          ) {
            specURLs.push(specURL);
          }
        } else {
...
const getSpecURLsArray = (
  ...
  const seconds = 20;
  const mdnJsonURL =
    'http://localhost:5000/en-US' + URL.parse(mdnURL).path + '/index.json';
  const options = {
  ...
  let specTable = '';
  const getSpecTable = (key, data) => {
    if (key === 'value') {
      if (data instanceof Object && 'title' in data) {
        if (data.title === 'Specifications') {
          specTable = data.content;
        }
      }
    }
    return data;
  ...
  try {
    let response = request('GET', mdnJsonURL, options);
    let statusCode = response.statusCode;
    if (statusCode >= 400) {
      response = request('GET', mdnURL + '/index.json', options);
      ...
      if (statusCode >= 400) {
        error(
          `${feature}: ${statusCode} ${mdnJsonURL} (unexpected status code)`,
        );
        return null;
      }
    ...
    JSON.parse(response.getBody('utf8'), getSpecTable);
    return scrapeMDNSpecTable(
      specTable,
      feature,
      mdnURL,
      deprecated,
    ...
    error(`${feature}: error for ${mdnJsonURL} ${e.message}.`);
    log(e);
  }
  return null;
};
...
const addSpecs = (key, data) => {
  ...
  if (data && data instanceof Object && '__compat' in data) {
    ...
    if (
      mdnURL ===
      'https://developer.mozilla.org/docs/Web/API/Window/clipboardchange_event'
    ) {
      return data;
    }
...
if (require.main === module) {
  ...
  if (process.argv[2] && process.argv[2] !== 'fullupdate') {
    ...
    load('api', 'css', 'http', 'mathml', 'svg', 'webdriver', 'xpath', 'xslt');
  }
}
